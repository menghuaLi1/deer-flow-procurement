...
- Extends `AgentState` with: `sandbox`, `thread_data`, `title`, `artifacts`, `todos`, `uploaded_files`, `viewed_images`, `procurement`
- Uses custom reducers: `merge_artifacts` (deduplicate), `merge_viewed_images` (merge/clear), `replace_procurement_state` (domain snapshot replacement)
- `procurement` stores the procurement workspace state rendered by the frontend: stage, project conditions, requirements, candidates, evidence, decision, pending questions, metrics, and sources
...
**Procurement tools** (`packages/harness/deerflow/procurement/`):
- `procurement_parse_requirements` - Parses uploaded/text BOQ, BOM, and procurement lists into structured requirement rows and missing-condition questions
- `procurement_save_state` - Saves the complete `procurement` thread-state snapshot for frontend rendering after sourcing/evidence/decision stages
- `procurement_render_report` - Writes Markdown and JSON procurement decision artifacts to `/mnt/user-data/outputs`
- The built-in `procurement-agent` uses tool groups `web`, `file:read`, `file:write`, and `procurement`; public-web evidence must include URLs, and unverifiable supplier claims should remain pending manual confirmation
